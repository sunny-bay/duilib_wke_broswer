...
DUI_BEGIN_MESSAGE_MAP(WindowImplBase,CNotifyPump)
	...
	DUI_ON_MSGTYPE(DUI_MSGTYPE_URLCHANGED, OnUrlChanged)
	DUI_ON_MSGTYPE(DUI_MSGTYPE_TITLECHANGED,	 OnTitleChanged)
DUI_END_MESSAGE_MAP()
...
void WindowImplBase::OnClose()
{
}

void WindowImplBase::OnDestroy()
{
}
...
LRESULT WindowImplBase::OnNcHitTest(POINT pt)
{
...
BOOL WindowImplBase::OnMouseWheel(UINT nFlags, short zDelta, POINT point)
{
	return TRUE;
}
...
void WindowImplBase::OnDropFiles(HDROP hDropInfo)
{

}

void WindowImplBase::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{

}

void WindowImplBase::OnSize(UINT nType, int cx, int cy)
{
...
BOOL WindowImplBase::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void WindowImplBase::OnKillFocus(HWND hNew)
{
}

void WindowImplBase::OnSetFocus(HWND hOld)
{
}


void WindowImplBase::OnLButtonDown(UINT nFlags, POINT point)
{

}

void WindowImplBase::OnLButtonUp(UINT nFlags, POINT point)
{

}

void WindowImplBase::OnMouseMove(UINT nFlags, POINT point)
{
}
...
LRESULT WindowImplBase::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
	case WM_CREATE:			lRes = OnCreate(LPCREATESTRUCT(lParam)) ? 0 : -1; bHandled = FALSE; break;
	case WM_CLOSE:			OnClose(); bHandled = FALSE; break;
	case WM_DESTROY:		OnDestroy(); bHandled = FALSE; break;
#if defined(WIN32) && !defined(UNDER_CE)
	case WM_NCACTIVATE:		lRes = OnNcActivate(uMsg, wParam, lParam, bHandled); break;
	case WM_NCCALCSIZE:		lRes = OnNcCalcSize(uMsg, wParam, lParam, bHandled); break;
	case WM_NCPAINT:		lRes = OnNcPaint(uMsg, wParam, lParam, bHandled); break;
	case WM_NCHITTEST:		{POINT pt = {LOWORD(lParam), HIWORD(lParam)}; lRes = OnNcHitTest(pt); break;}
	case WM_GETMINMAXINFO:	lRes = OnGetMinMaxInfo(uMsg, wParam, lParam, bHandled); break;
	case WM_MOUSEWHEEL:		{ POINT pt = {LOWORD(lParam), HIWORD(lParam)}; lRes = OnMouseWheel(LOWORD(wParam), HIWORD(wParam), pt); bHandled = FALSE; } break;
#endif
	case WM_DROPFILES:			OnDropFiles(HDROP(wParam)); bHandled = FALSE; break;
	case WM_SIZE:			OnSize(wParam, LOWORD(lParam), HIWORD(lParam)); bHandled = FALSE; break;
	case WM_CHAR:		
	...
	case WM_KILLFOCUS:		OnKillFocus(HWND(wParam)); bHandled = FALSE; break;
	case WM_SETFOCUS:		OnSetFocus(HWND(wParam)); bHandled = FALSE; break;
	case WM_LBUTTONUP:		{ POINT pt = {LOWORD(lParam), HIWORD(lParam)}; OnLButtonUp(wParam, pt); bHandled = FALSE; } break;
	case WM_LBUTTONDOWN:		{ POINT pt = {LOWORD(lParam), HIWORD(lParam)}; OnLButtonDown(wParam, pt); bHandled = FALSE; } break;
	case WM_MOUSEMOVE:		{ POINT pt = {LOWORD(lParam), HIWORD(lParam)}; OnMouseMove(wParam, pt); bHandled = FALSE; } break;
...
void WindowImplBase::OnTimer(TNotifyUI& msg)
{
}
void WindowImplBase::OnMenu(TNotifyUI& msg)
{
}
void WindowImplBase::OnReturn(TNotifyUI& msg)
{
}
void WindowImplBase::OnUrlChanged(TNotifyUI& msg)
{
}
void WindowImplBase::OnTitleChanged(TNotifyUI& msg)
{
}
