...
public:
	...
	void paint(HDC hdc, int x, int y, int cx, int cy, int xSrc, int ySrc, bool fKeepAlpha);
	void repaintIfNeeded();
	HDC viewDC();
	virtual HDC getViewDC(){ return viewDC(); }
	...
	virtual bool mouseEvent(unsigned int message, int x, int y, unsigned int flags){ return fireMouseEvent(message, x, y, flags); };
	virtual bool contextMenuEvent(int x, int y, unsigned int flags){ return fireContextMenuEvent(x, y, flags); };
	virtual bool mouseWheel(int x, int y, int delta, unsigned int flags) { return fireMouseWheelEvent(x, y, delta, flags); };
	virtual bool keyUp(unsigned int virtualKeyCode, unsigned int flags, bool systemKey){ return fireKeyUpEvent(virtualKeyCode, flags, systemKey); };
	virtual bool keyDown(unsigned int virtualKeyCode, unsigned int flags, bool systemKey){ return fireKeyDownEvent(virtualKeyCode, flags, systemKey); };
	virtual bool keyPress(unsigned int virtualKeyCode, unsigned int flags, bool systemKey){ return fireKeyPressEvent(virtualKeyCode, flags, systemKey); };
	...
	virtual void unfocus(){ killFocus(); };

	bool cookieEnabled() const{ return isCookieEnabled(); };

	wkeRect caretRect();
	virtual wkeRect getCaret(){ return caretRect(); };
	...
	//////////////////////扩展////////////////////////////////
	void simulatedHitTestElement(const wchar_t* id);
	const char* getReferer();
	void setReferer(const wchar_t * refer);
	HTMLElement getElementById(const wchar_t*);
	void  getElementByName(ElementVector eleVec, const wchar_t*);
	void  getElementByTagAndAttr(ElementVector eleVec, const wchar_t* tagName, const wchar_t* attr);
