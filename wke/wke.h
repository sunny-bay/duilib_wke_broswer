...
WKE_API void WKE_CALL wkeShutdown();
...
WKE_API const utf8* WKE_CALL wkeToString(const wkeString string);
WKE_API const wchar_t* WKE_CALL wkeToStringW(const wkeString string);
...
namespace wke
{
	...
	typedef vector<HTMLElement> ElementVector;//用一个容器来存放元素指针
	class IWebView
	{
	public:
		...
		virtual void loadPostURL(const utf8* inUrl, const char * poastData, int nLen) = 0;
		virtual void loadPostURL(const wchar_t * inUrl, const char * poastData, int nLen) = 0;

		virtual void loadURL(const utf8* url) = 0;
		virtual void loadURL(const wchar_t* url) = 0;

		virtual void load(const utf8* str) = 0;
		virtual void load(const wchar_t* str) = 0;

		virtual bool isLoadingSucceeded() const = 0;
		virtual bool isLoadingFailed() const = 0;
		...
		virtual int contentWidth() const = 0;
		virtual int contentHeight() const = 0;

		virtual void setFocus() = 0;
		virtual void killFocus() = 0;

		virtual void onNavigation(wkeNavigationCallback callback, void* callbackParam) = 0;
		virtual void onCreateView(wkeCreateViewCallback callback, void* callbackParam) = 0;

		virtual void onConsoleMessage(wkeConsoleMessageCallback callback, void* callbackParam) = 0;
		virtual void onLoadingFinish(wkeLoadingFinishCallback callback, void* callbackParam)=0;
		...
		virtual bool fireKeyPressEvent(unsigned int charCode, unsigned int flags, bool systemKey) = 0;


		virtual void onAlertBox(wkeAlertBoxCallback callback, void* callbackParam) = 0;
		virtual void onConfirmBox(wkeConfirmBoxCallback callback, void* callbackParam) = 0;
		virtual void onPromptBox(wkePromptBoxCallback callback, void* callbackParam) = 0;
		...
		virtual bool isLoadingCompleted() const = 0;
		...
		virtual void setHostWindow(HWND win) = 0;
		virtual HWND hostWindow() const = 0;

		virtual wkeRect caretRect()=0;
		virtual void wake()=0;

		virtual void onURLChanged(wkeURLChangedCallback callback, void* callbackParam) =0;
		virtual void onTitleChanged(wkeTitleChangedCallback callback, void* callbackParam)=0;
		virtual void onPaintUpdated(wkePaintUpdatedCallback callback, void* callbackParam) =0;

		virtual void onDocumentReady(wkeDocumentReadyCallback callback, void* callbackParam)=0;

		virtual void tick() =0;
		//////////////////////扩展///////////////////////
		virtual const char* getReferer()=0;
		virtual void setReferer(const wchar_t * refer)=0;
		virtual void setUserAgent(const wchar_t * useragent) = 0;
		virtual void simulatedHitTestElement(const wchar_t*)=0;
		virtual HTMLElement getElementById(const wchar_t*) = 0;

		virtual void  getElementByName(ElementVector eleVec,const wchar_t*) = 0;
		virtual void  getElementByTagAndAttr(ElementVector eleVec, const wchar_t* tagName, const wchar_t* attr) = 0;
	};
}
...
WKE_API const utf8* WKE_CALL jsToString(jsExecState es, jsValue v);
WKE_API const wchar_t* WKE_CALL jsToStringW(jsExecState es, jsValue v);
